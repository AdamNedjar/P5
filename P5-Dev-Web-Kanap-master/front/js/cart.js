...
if (localStorageProducts == null || localStorageProducts.length == 0) {
  ...
  document.querySelector(
    ".cart__price"
  ).innerHTML = `<p>Total (<span id="totalQuantity">0</span> articles) : <span id="totalPrice">0</span> €</p>`;
...
function updateCart() {
  ...
  for (let i = 0; i < localStorageProducts.length; i++) {
    ...
    let priceProduitDansLePanier =
      localStorageProducts[i].price * localStorageProducts[i].quantity;
  ...
  document.querySelector(
    ".cart__price"
  ).innerHTML = `<p>Total (<span id="totalQuantity">${quantityTotalCalcul}</span> articles) : <span id="totalPrice">${priceTotalCalcul.toFixed(
    2
  )} €</span></p>`;
}
...
function modifValue() {
  ...
  let inputQuantity = Array.from(
    document.querySelectorAll(".cart__item__content__settings__quantity input")
  );
...
function deleteProduct(index) {
  ...
  const cartItem = document.querySelector(
    `.cart__item[data-id="${localStorageProducts[index].id}"][data-color="${localStorageProducts[index].colors}"]`
  );
  cartItem.remove();
